...
class _P2Analyzer(Visitor):
    ...
    def visit_return_statement(self, return_statement: ReturnStatement) -> None:
        ...
        if len(return_statement.transform_list) == 0:
...
class _P3Analyzer:
    ...
    def _reduce_and_exprs(self, and_exprs: list[AndExpr]) -> list[AndExpr] | None:
        ...
        for i, and_expr in enumerate(and_exprs):
            ...
            test_expr_key_sets[i] = set(self._make_test_expr_keys(test_exprs))
            small_and_exprs[i] = and_expr
        ...
        for i, and_expr in enumerate(small_and_exprs_2):
            test_exprs = self._merge_test_exprs(and_expr.test_exprs)
            small_and_exprs_2[i] = dataclasses.replace(and_expr, test_exprs=test_exprs)

        return small_and_exprs_2
    ...
    def _make_test_expr_keys(cls, test_exprs: list[TestExpr]) -> Iterator[int]:
        for test_expr in test_exprs:
            yield test_expr.test_id

    def _rearrange_and_exprs(self, and_exprs: list[AndExpr]) -> None:
        n = len(self._absolute_test_ids)
        test_id_ref_counts: list[int] = (2 * n + 1) * [0]
        for and_expr in and_exprs:
            for test_expr in and_expr.test_exprs:
                test_id_ref_counts[n + test_expr.test_id] += 1

        def and_expr_rank(and_expr: AndExpr) -> Iterator[int]:
            for test_expr in and_expr.test_exprs:
                test_id_ref_count = test_id_ref_counts[n + test_expr.test_id]
